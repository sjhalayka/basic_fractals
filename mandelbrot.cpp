...
float iterate_Mandelbrot_4d(vector< quaternion >& trajectory_points,
    const quaternion C,
...
float iterate_Julia_4d(vector< quaternion >& trajectory_points,
    quaternion Z,
    const quaternion C,
    const short unsigned int max_iterations,
    const float threshold)
    ...
    for (size_t z = 0; z < z_res; z++, Z.z += z_step_size)
    {
        ...
        for (size_t x = 0; x < x_res; x++, Z.x += x_step_size)
        {
            ...
            for (size_t y = 0; y < y_res; y++, Z.y += y_step_size)
            {
                ...
                // If Mandelbrot set
                float magnitude = iterate_Mandelbrot_4d(trajectory_points, Z, max_iterations, threshold);
                
                // Else Julia set
                //float magnitude = iterate_Julia_4d(trajectory_points, Z, C, max_iterations, threshold);
